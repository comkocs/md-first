...
python -m pytest tests -q                    # 540 条全绿（成绩随「在哪跑」变，见下表）
...
|---|---|---|
| 完整仓树 | `540 passed` | 无 |
| 完整仓树，但包目录下有 `config.json` | `539 passed, 1 skipped` | 「包目录下已有 `config.json`（运行者自己的），不覆盖它」 |
| 上服包 + 服务器（root、无 node） | `534 passed, 6 skipped` | 1 条「root 下会真去动系统目录」+ 5 条「这台机器上没有 node」 |
...
这条排在所有设计之前，因为它是用来**否决**其它设计的。

> **凡是要用的人自己记得去做、去授权、去补一条命令的地方，都算这个工具没做好。**
> 不是用的人不够细心——**用的人本来就不该懂这些**。

它长成什么样，举三个这个工具自己犯过的：

- 收件箱读完不自动标已读，于是「要你去唤醒的窗口」永远亮着，
  而修法一度是「收工时记得多跑一条 `--mark-read`」——**让人记住一件事，不是修法**。
- 开窗指令里要人手抄一串本机约束（先建工作树、走哪个代理、别在仓根写）。
  抄漏一条的后果要到半小时后才出现。
- 输出编码要人自己去设环境变量——而他得先看得懂那句乱码，才知道要去设。

**判据**：写文档之前先问一句——「这一步能不能让程序自己做掉？」
能，就去做掉；不能，再写进文档。**文档是最后手段，不是第一手段。**

★反过来也有一条界限：**该停下来问的一定要问。**
删东西、改线上、不可逆的动作，问一句是对的。
这条原则针对的是**重复的、机械的、每次都一样的那些步骤**——
那些步骤问一百次，答案一百次都相同，那就不该问。
...
外网走不走代理、哪些端口不许碰），写进 config 的 `本机约束`，
**每张派单的第二行会自动带上**：

```jsonc
"本机约束": [
  "先建自己的工作树：git worktree add _work/wt-<你的编号> -b <分支> origin/main，之后一切命令在自己树里",
  "外网走代理 127.0.0.1:7890，不要自己去扫端口",
  "输出编码：PowerShell 不认 && ，多条命令用 ; 分开"
]
```
